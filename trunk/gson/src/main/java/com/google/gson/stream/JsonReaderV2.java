 ...
 * @since 1.6
 */
public class JsonReaderV2 implements Closeable {
  ...
  private int doPeek() throws IOException {
    ...
    if (peekStack == JsonScope.EMPTY_ARRAY) {
      ...
      // TODO: support '=' and "=>" delimiters in lenient mode.
      int c = nextNonWhitespace(true);
      switch (c) {
      case ':':
        break;
  ...
  private int peekKeyword() throws IOException {
    // TODO: figure out a way to avoid repeatedly accessing 'buffer'
    int c;
    if ((pos + 4 < limit || fillBuffer(5))
        && ((c = buffer[pos    ]) == 't' || c == 'T')
        && ((c = buffer[pos + 1]) == 'r' || c == 'R')
        && ((c = buffer[pos + 2]) == 'u' || c == 'U')
        && ((c = buffer[pos + 3]) == 'e' || c == 'E')
        && !isLiteral(buffer[pos + 4])) {
      pos += 4;
      return peeked = PEEKED_TRUE;
    } else if ((pos + 5 < limit || fillBuffer(6))
        && ((c = buffer[pos    ]) == 'f' || c == 'F')
        && ((c = buffer[pos + 1]) == 'a' || c == 'A')
        && ((c = buffer[pos + 2]) == 'l' || c == 'L')
        && ((c = buffer[pos + 3]) == 's' || c == 'S')
        && ((c = buffer[pos + 4]) == 'e' || c == 'E')
        && !isLiteral(buffer[pos + 5])) {
      pos += 5;
      return peeked = PEEKED_FALSE;
    } else if ((pos + 4 < limit || fillBuffer(5))
        && ((c = buffer[pos    ]) == 'n' || c == 'N')
        && ((c = buffer[pos + 1]) == 'u' || c == 'U')
        && ((c = buffer[pos + 2]) == 'l' || c == 'L')
        && ((c = buffer[pos + 3]) == 'l' || c == 'L')
        && !isLiteral(buffer[pos + 4])) {
      pos += 4;
      return peeked = PEEKED_NULL;
    } else {
  ...
  private int peekNumber() throws IOException {
    ...
    if (c == '0') {
      ...
      c = get(++i);
      while (c >= '0' && c <= '9') {
        integer = integer * 10 - (c - '0'); // TODO: detect overflow
        c = get(++i);
      }
    } else {
      return PEEKED_NONE;
  ...
  private String nextUnquotedValue() throws IOException {
    ...
    String result;
    if (false /* TODO: support skipping */) {
      result = "skipped!";
    } else if (builder == null) {
